...
export default function ChatbotPage() {
  ...
  const processQuery = (query: string) => {
    ...
    if (lowerQuery.includes("pending") || lowerQuery.includes("due")) {
      const salesPending = db.getSalesBills().filter(b => !b.isPaid && b.advanceAmount === 0);
      const rentalPending = db.getRentalBills().filter(b => !b.isPaid && b.advanceAmount === 0);
      const totalPending = salesPending.reduce((sum, b) => sum + b.totalAmount, 0) +
                          rentalPending.reduce((sum, b) => sum + b.totalAmount, 0);

      return `Pending Payments Summary:\n\n• Sales: ${salesPending.length} bills - ₹${salesPending.reduce((sum, b) => sum + b.totalAmount, 0).toFixed(2)}\n• Rental: ${rentalPending.length} bills - ₹${rentalPending.reduce((sum, b) => sum + b.totalAmount, 0).toFixed(2)}\n• Total Pending: ₹${totalPending.toFixed(2)}\n\nYou can view detailed pending bills in the Pending section.`;
    }
    ...
    if (lowerQuery.includes("stats") || lowerQuery.includes("statistics") || lowerQuery.includes("total") || lowerQuery.includes("revenue")) {
      ...
      const salesTotal = salesBills.reduce((sum, b) => sum + (b.totalAmount + b.advanceAmount), 0);
      const rentalTotal = rentalBills.reduce((sum, b) => sum + (b.totalAmount + b.advanceAmount), 0);

      return `Business Statistics:\n\n📊 Sales:\n• Total Bills: ${salesBills.length}\n• Revenue: ₹${salesTotal.toFixed(2)}\n• Fully Paid: ${salesBills.filter(b => b.isPaid).length}\n\n🏠 Rental:\n• Total Bills: ${rentalBills.length}\n• Revenue: ₹${rentalTotal.toFixed(2)}\n• Fully Paid: ${rentalBills.filter(b => b.isPaid).length}\n\n💰 Total Revenue: ₹${(salesTotal + rentalTotal).toFixed(2)}`;
    }

    // Recent bills
    if (lowerQuery.includes("recent") || lowerQuery.includes("latest") || lowerQuery.includes("last")) {
      const allBills = [
        ...db.getSalesBills().map(b => ({ ...b, type: "Sales" })),
        ...db.getRentalBills().map(b => ({ ...b, type: "Rental" }))
